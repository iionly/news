...
for ($i = 1; $i < 6; $i++) {
	...
	echo elgg_view("input/select", array("options_values" => $options_values, "name" => "params[" . $metadata_name . "]", "value" => $widget->$metadata_name));
	echo "</div>";
}

echo "<div>";
echo elgg_echo("news:news_in_groups:settings:news_count") . " ";
echo elgg_view("input/select", array("options" => array(1,2,3,4,5,10,15,20), "name" => "params[news_count]", "value" => $news_count));
echo "</div>";

echo "<div>";
echo elgg_echo("news:news_in_groups:settings:group_icon_size") . " ";
echo elgg_view("input/select", array("options_values" => array("medium" => elgg_echo("news:news_in_groups:settings:group_icon_size:medium"), "small" => elgg_echo("news:news_in_groups:settings:group_icon_size:small")), "name" => "params[group_icon_size]", "value" => $widget->group_icon_size));
echo "</div>";
